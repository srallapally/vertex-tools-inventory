...
from inventory.collectors.webhooks import (
...
from inventory.normalize.tool_credentials import normalize_tool_credentials
...
def run(config: InventoryConfig) -> None:
    ...
    if config.fixtures:
        ...
        if config.flavor in {"dialogflowcx", "both"} and config.tool_credentials_fixture_path is not None:
            raw_webhooks = collect_webhooks_from_fixture(config.tool_credentials_fixture_path)
    else:
        if config.flavor == "dialogflowcx":
        ...
        if config.flavor in {"dialogflowcx", "both"}:
            raw_webhooks = collect_webhooks_live(agents)
    ...
    write_tool_credentials_json(config.output_dir, tool_credentials)
    write_manifest_json(
        config.output_dir,
        ...
        warnings,
        tool_credential_count=len(tool_credentials),
    )
